...
最近翻译了IETF的QUIC协议提案，在翻译的过程中对QUIC协议有了更深入的理解。同时也思考了一些与QUIC相关的问题，尤其是针对如何在IoT设备环境中实施QUIC协议。
...
其次，不同于传统TCP使用<源IP，源端口，目标IP，目标端口>四元组来表示连接，QUIC使用连接ID标记连接，通过这样的方法，两个节点间的连接可以轻易的迁移，解决UDP穿越NAT数据包改写的问题，甚至能保证节点切换网络后连接也能继续进行。同时由于QUIC的0-RTT特性，当连接迁移时，不需重新对连接的密钥进行重新协商。
...
在IoT环境下，TLS开销较大，系统过于复杂。TLS中包含两部分，分别是：用于加密认证传输TLS数据包的记录层，和用于交换密钥，保证安全性的密钥握手协议。这两个层次分别侧重于不同的密码学机制，记录层集中使用对称加密机制，而密钥握手协议则集中于非对称加密机制。当我们寻找替代协议时，我们需要找到适用这两个层次的密码学协议。

幸运的是，我们已经有了十分轻量的可用的密码学协议：

Strobe是一个关注与对称加密的密码学协议框架，基于Keccak构建。

Noise是一个非对称握手与密钥交换的协议框架，基于25519构建。
